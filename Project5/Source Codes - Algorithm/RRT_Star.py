            ...
            if def_val < count_Points_Nodes:
                ...
                while Consec_point == False:
                    ...
                    for p in Points_Nodes:
                        if Length(p.pts, rand) <= Length(parentPoints_Node.pts, rand) and Length(p.pts, rand2) <= Length(parentPoints_Node.pts, rand2) and Length(p.pts, rand3) <= Length(parentPoints_Node.pts, rand3):
                            newPoint = Func_step_to(p.pts, rand)
                            newPoint2 = Func_step_to(p.pts, rand2)
                            newPoint3 = Func_step_to(p.pts, rand3)
                            if obstacle_coll(newPoint) == False and obstacle_coll(newPoint2) == False and obstacle_coll(newPoint3) == False:
                                parentPoints_Node = p
                                Consec_point = True

                newPoints_Node = Func_step_to(parentPoints_Node.pts, rand)
                newPoints_Node2 = Func_step_to(parentPoints_Node.pts, rand2)
                newPoints_Node3 = Func_step_to(parentPoints_Node.pts, rand3)
                Points_Nodes.append(Points_Node(newPoints_Node, parentPoints_Node))
                Points_Nodes.append(Points_Node(newPoints_Node2, parentPoints_Node))
                Points_Nodes.append(Points_Node(newPoints_Node3, parentPoints_Node))

                pygame.draw.line(screen, cyan, parentPoints_Node.pts, newPoints_Node)

                if Col_circle(newPoints_Node, Pt_goal.pts, bound_radius) or Col_circle(newPoints_Node2, Pt_goal.pts, bound_radius) or Col_circle(newPoints_Node3, Pt_goal.pts, bound_radius):
                ...
                if Present_state == 'init':
                    if ValInit == False:
                        ...
                        if obstacle_coll(e.pos) == False:
                            Firstpoint = Points_Node((241,250-20), None)
                        ...
                        if obstacle_coll(e.pos) == False:
                            Pt_goal = Points_Node((6,250-157), None)
                            FinalInit = True
