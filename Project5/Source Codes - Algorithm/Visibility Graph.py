...
def ConnectVertices(edge):
    v = edge.p1
    w = edge.p2
    plt.plot([v.x, w.x], [v.y, w.y], 'b--')

def GenerateDistance(path):
    for i in range(len(path) - 1):
        v = path[i]
        w = path[i + 1]
        plt.plot([v.x, w.x], [v.y, w.y], 'g-',linewidth=1.0)


Polygons = [[vg.Point(69.0,88.0), vg.Point(69.0,165.0),vg.Point(182.0,165.0), vg.Point(182.0,88.0)],
...
shortest = g.shortest_path(vg.Point(6,248), vg.Point(239,6))

plt.subplot(1,2,1)
GenerateDistance(shortest)
...
length = "{}".format(Distance(shortest))
